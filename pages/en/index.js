  ...
  return (
    <div className="bg-teal-900 min-h-screen">
      <div className='container mx-auto py-14 px-6'>
        ...
        <h1 className='text-5xl font-extrabold text-white mb-5'>FavMusicList
          {locales !== undefined && locales.map((l, i) => {
            return (
              <span key={i} >
                <Link href={asPath} locale={l} className={`font-bold text-lg text-right text-teal-200 ml-3 ${l === locale ? 'border-b-2 border-teal-100' : ""}`}>
                  {l}
                </Link>
              </span>
            );
